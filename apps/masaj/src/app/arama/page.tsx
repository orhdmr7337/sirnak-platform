    ...
    <SiteProvider data={data}>
      ...
      <main className="pt-28 pb-20">
        <div className="max-w-4xl mx-auto px-6">
          ...
          {query.length < 2 && (
            <p className="mt-4 text-gray-400">En az 2 karakter girerek arama yapın.</p>
          )}
          ...
          {query.length >= 2 && results.length === 0 && (
            <div className="mt-12 text-center">
              ...
              <p className="mt-4 text-lg text-gray-400">Sonuç bulunamadı</p>
              <p className="mt-2 text-sm text-gray-500">Farklı anahtar kelimelerle tekrar deneyin.</p>
            </div>
          )}

          {Object.entries(groupedResults).map(([type, items]) => (
            <div key={type} className="mt-10">
              <h2 className="mb-4 text-lg font-semibold text-gray-400">{typeLabels[type] ?? type}</h2>
              <div className="space-y-3">
                {items.map((item) => (
                  <a
                    key={`${item.type}-${item.id}`}
                    href={item.url}
                    className="block rounded-xl border border-[#2a3a2a]/50 bg-[#0a0f0a]/50 p-5 transition-all hover:border-[#6b8f71]/30 hover:bg-[#0a0f0a]/80"
                  >
                    <div className="flex items-start gap-4">
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[#6b8f71]/10 text-[#6b8f71]">
